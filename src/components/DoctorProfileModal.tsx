            ...
            {activeTab === 'credentials' && (
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-slate-300 mb-1.5 flex items-center gap-1.5">
                      ...
                      ABDM HPR ID (NHA sandbox path)
                    </label>
                    <input
                      type="text"
                      value={formData.hprId || ''}
                      onChange={(e) => setFormData({ ...formData, hprId: e.target.value })}
                      placeholder="local placeholder, e.g. HPR-IN-2012-9841"
                      className="w-full px-3 py-2 bg-slate-800/80 border border-slate-700 rounded-lg text-xs text-white placeholder-slate-500 focus:outline-none focus:border-blue-500 font-mono"
                    />
                  </div>
                </div>
                ...
                <div className="p-3.5 bg-blue-500/10 border border-blue-500/20 rounded-xl text-xs text-blue-300 flex items-start gap-2.5">
                  ...
                  <div>
                    <span className="font-semibold block">ABDM-aligned records (NHA sandbox):</span>
                    Registration numbers and HPR IDs are copied into prescriptions when present. Locally generated HPR values are pending — not yet verified with the National Health Authority. Contact ravee@lumer.me for NHA sandbox credential questions.
                  </div>
                </div>
              </div>
            )}
