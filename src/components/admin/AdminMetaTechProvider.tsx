    ...
    <div className="space-y-6 max-w-6xl mx-auto pb-12">
      ...
      {activeSubTab === "review" && (
        <div className="space-y-6">
          <div className="bg-white rounded-xl p-6 border border-slate-200 shadow-sm space-y-4" data-testid="admin-meta-platform-credentials">
            <div className="flex items-center justify-between flex-wrap gap-2">
              ...
              <span className="px-3 py-1 bg-amber-50 text-amber-800 font-semibold text-xs rounded-full border border-amber-200">
                Env-configured — secrets masked
              </span>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
              <div className="p-3 rounded-lg border border-slate-200 bg-slate-50">
                <div className="text-[11px] uppercase tracking-wide text-slate-500 font-semibold">Meta App ID</div>
                <code className="text-xs font-mono text-slate-800 break-all">{platform?.appId || "not set"}</code>
              </div>
              <div className="p-3 rounded-lg border border-slate-200 bg-slate-50">
                <div className="text-[11px] uppercase tracking-wide text-slate-500 font-semibold">App Secret</div>
              ...
              <div className="p-3 rounded-lg border border-slate-200 bg-slate-50">
                ...
                <code className="text-xs font-mono text-slate-800 break-all">
                  {platform?.fallbackPhoneNumberId || platform?.phoneNumberId || "not set"}
                </code>
              </div>
              <div className="p-3 rounded-lg border border-slate-200 bg-slate-50 md:col-span-2">
                <div className="text-[11px] uppercase tracking-wide text-slate-500 font-semibold">Central webhook receiver</div>
                <div className="flex items-center gap-2">
                  <code className="text-xs font-mono text-slate-800 break-all">{platform?.webhookUrl || webhookUrl}</code>
                  <button
                    type="button"
                    onClick={() => copyToClipboard(platform?.webhookUrl || webhookUrl, "platform-webhook")}
                    className="p-1.5 text-slate-600 hover:text-slate-900 hover:bg-slate-200 rounded"
                    title="Copy webhook URL"
                  >
                    <Copy className="w-4 h-4" />
                  </button>
                  {copiedKey === "platform-webhook" ? <span className="text-[11px] text-emerald-700">Copied</span> : null}
                </div>
                <p className="text-[11px] text-slate-500 mt-1">
                  Verify token: {platform?.webhookVerifyTokenConfigured ? "configured (META_VERIFY_TOKEN)" : "not set"} ·
                  Embedded Signup config: {platform?.embeddedSignupConfigIdConfigured ? "configured" : "not set"}
                </p>
              </div>
            </div>
            {platform?.notice ? <p className="text-xs text-slate-500">{platform.notice}</p> : null}
          </div>

          <div className="bg-white rounded-xl p-6 border border-slate-200 shadow-sm space-y-4">
            <div>
              <h2 className="text-lg font-bold text-slate-900">Tenant WhatsApp onboarding path</h2>
              <p className="text-sm text-slate-500">
                Clinics complete Meta Embedded Signup themselves. This Super Admin surface only shows platform credentials
                and the resulting WABA inventory.
              </p>
            </div>
            <ol className="space-y-3 text-sm text-slate-700">
              <li className="p-3 rounded-lg border border-slate-200 bg-slate-50">
                <div className="font-semibold text-slate-900">1. Clinic opens Settings → Integrations</div>
                <p className="text-xs text-slate-500 mt-1">
                  Path aliases: <code>/settings/integrations</code> and the onboarding wizard. There are no token or API-secret
                  inputs on the tenant form.
                </p>
              </li>
              <li className="p-3 rounded-lg border border-slate-200 bg-slate-50">
                <div className="font-semibold text-slate-900">2. FB.login Embedded Signup v4</div>
                <p className="text-xs text-slate-500 mt-1">
                  Scopes requested: <code>whatsapp_business_management</code> and <code>whatsapp_business_messaging</code>.
                  The session returns an authorization code plus WABA ID, phone number ID, and business ID.
                </p>
              </li>
              <li className="p-3 rounded-lg border border-slate-200 bg-slate-50">
                <div className="font-semibold text-slate-900">3. Tenant-only token exchange</div>
                <p className="text-xs text-slate-500 mt-1">
                  <code>POST /api/integrations/whatsapp/embedded-signup</code> is restricted to clinic roles (doctor,
                  polyclinic_admin, CLINIC_ADMIN). Super Admin receives 403 and must not exchange tenant codes.
                </p>
              </li>
              <li className="p-3 rounded-lg border border-slate-200 bg-slate-50">
                <div className="font-semibold text-slate-900">4. Central webhook subscription</div>
                <p className="text-xs text-slate-500 mt-1">
                  After code exchange, Lumera POSTs <code>/{`{waba-id}`}/subscribed_apps</code> so the new WABA delivers
                  events to <code>/api/meta/webhook</code>.
                </p>
              </li>
              <li className="p-3 rounded-lg border border-slate-200 bg-slate-50">
                <div className="font-semibold text-slate-900">5. Shared test number while Meta verifies</div>
                <p className="text-xs text-slate-500 mt-1">
                  Appointment reminders send from <code>META_FALLBACK_PHONE_NUMBER_ID</code> (or{" "}
                  <code>META_PHONE_NUMBER_ID</code>) until the clinic phone reports <code>VERIFIED</code> and the display
                  name is <code>APPROVED</code>. Contact ravee@lumer.me. Lumera is not a Meta Tech Provider.
                </p>
              </li>
            </ol>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
              <div className="p-3 rounded-lg border border-amber-200 bg-amber-50 text-amber-900">
                <div className="font-semibold mb-1">Tenant UI must show</div>
                <ul className="list-disc pl-4 space-y-0.5">
                  <li>Meta Business Verification badge</li>
                  <li>WABA ID badge</li>
                  <li>Phone Number Status badge</li>
                  <li>Fallback-number notice while verification is pending</li>
                </ul>
              </div>
              <div className="p-3 rounded-lg border border-slate-200 bg-slate-50 text-slate-700">
                <div className="font-semibold mb-1">Super Admin must show</div>
                <ul className="list-disc pl-4 space-y-0.5">
                  <li>Meta App ID (plain)</li>
                  <li>App Secret (masked preview only)</li>
                  <li>System user / Tech Provider-style token (masked)</li>
                  <li>Central webhook subscription URL</li>
                </ul>
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-900 text-slate-100 rounded-xl p-6 border border-slate-800 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs text-slate-300">
              <div className="p-3.5 bg-slate-800/80 rounded-lg border border-slate-700/70">
                <div className="font-semibold text-amber-300 mb-1">Embedded Signup — tenant only</div>
                <p>
                  Clinic users complete Meta Embedded Signup from Settings. This Super Admin console does not launch FB.login
                  or exchange tenant authorization codes.
                </p>
              </div>
              <div className="p-3.5 bg-slate-800/80 rounded-lg border border-slate-700/70">
                <div className="font-semibold text-amber-300 mb-1">WABA inventory — read path</div>
                <p>
                  Connected practice WABAs listed here are linked by tenant Embedded Signup. Demo tokens with <code>EAAJ...</code>{" "}
                  are not live Graph credentials.
                </p>
              </div>
              <div className="p-3.5 bg-slate-800/80 rounded-lg border border-slate-700/70">
                <div className="font-semibold text-amber-300 mb-1">System user tokens — env</div>
                <p>
                  Platform App ID, App Secret, and META_GRAPH_TOKEN (alias META_ACCESS_TOKEN) live in the runtime environment. Configure them for Graph
                  OTP and the shared test number used while a clinic awaits Meta verification.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-slate-500">
                Inventory of tenant-linked WhatsApp Business Account IDs. Clinics connect through Embedded Signup; this
                console does not launch Meta Login. Quality ratings shown here are stored copies — Graph GREEN is not
                inferred live.
              </p>
            <p className="text-xs text-slate-400 mt-1">
              To relink a practice, the clinic admin must re-run Embedded Signup from <code>/settings/integrations</code>.
              Super Admin does not paste system user tokens into a tenant record from this screen.
            </p>
            <p className="text-xs text-slate-400">
              Shared test number: appointment reminders for unverified display names go through{" "}
              <code>META_FALLBACK_PHONE_NUMBER_ID</code> until Meta returns <code>VERIFIED</code> / <code>APPROVED</code>.
              This is a sandbox-honest fallback, not a claim that Lumera is a Meta Tech Provider.
            </p>
            <p className="text-xs text-slate-400">
              Webhook receiver stays centralized at <code>/api/meta/webhook</code>. Per-tenant callback URLs are not
              registered; subscribed_apps points every customer WABA at Lumera's single receiver.
            </p>
            <p className="text-xs text-slate-400">
              Founder contact for Meta app provisioning remains ravee@lumer.me. FACEBOOK_APP_ID and FACEBOOK_APP_SECRET
              stay env-only; this console never writes those secrets back to disk.
            </p>
            </div>
          </div>
                ...
                {wabas.map((w) => (
                  <tr key={w.tenantId} className="hover:bg-slate-50/80 transition">
                    ...
                    <td className="px-5 py-4 text-right space-x-2">
                      <span className="text-[11px] text-slate-400">Tenant-linked</span>
                      <span className="text-[11px] text-slate-400 block">No Embedded Signup from Super Admin</span>
                    </td>
                  </tr>
                ))}
              </tbody>
