          ...
          {DEMOS.map((d) => (
            <button
              ...
              onClick={async () => {
                ...
                setError("");
                try {
                  const signedIn = await login(d.email, "Lumera@2026");
                  const dest = allowedNext(loginNext, signedIn.role) ? loginNext : homeSurfaceForRole(signedIn.role);
                  go(dest);
                } catch (err) {
                  setError(err instanceof Error ? err.message : "Login failed");
              ...
              className="text-left rounded-xl border border-white/10 bg-black/20 p-3 hover:border-purple-400/60 hover:bg-purple-900/25 transition group cursor-pointer disabled:opacity-50"
            >
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-white group-hover:text-purple-300">{d.label}</span>
                <span className="text-[10px] text-purple-400 font-medium group-hover:translate-x-0.5 transition-transform">&rarr;</span>
              </div>
