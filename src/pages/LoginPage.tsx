...
const SOCIAL_OAUTH_BTN_CLASS =
  "flex items-center justify-center gap-2 min-h-11 h-11 py-2 px-3 rounded-xl bg-slate-950 hover:bg-slate-800/90 border border-slate-700/80 hover:border-slate-600 text-xs font-semibold text-slate-200 transition-all hover:shadow-sm disabled:opacity-50";

const GoogleMark: React.FC<{ className?: string }> = ({ className = "w-4 h-4" }) => (
...
const FacebookMark: React.FC<{ className?: string }> = ({ className = "w-4 h-4 fill-[#1877F2]" }) => (
  <svg className={className} viewBox="0 0 24 24" aria-hidden>
    <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
  </svg>
);

const SocialOAuthButtons: React.FC<{
  busy: boolean;
  oauthBusy: "google" | "facebook" | null;
  onGoogle: () => void;
  onFacebook: () => void;
  googleTestId: string;
  facebookTestId: string;
}> = ({ busy, oauthBusy, onGoogle, onFacebook, googleTestId, facebookTestId }) => (
  <div className="grid grid-cols-2 gap-3" data-testid="oauth-social-row">
    <button
      type="button"
      ...
      className={SOCIAL_OAUTH_BTN_CLASS}
    >
      {oauthBusy === "google" ? <RefreshCw className="w-4 h-4 animate-spin" /> : <GoogleMark />}
      <span>{oauthBusy === "google" ? "Connecting…" : "Google"}</span>
    </button>
    <button
      type="button"
      data-testid={facebookTestId}
      onClick={onFacebook}
      disabled={busy}
      ...
      className={SOCIAL_OAUTH_BTN_CLASS}
    >
      {oauthBusy === "facebook" ? <RefreshCw className="w-4 h-4 animate-spin" /> : <FacebookMark />}
...
export const LoginPage: React.FC = () => {
  ...
  const bootOauth =
    typeof window !== "undefined" ? unregisteredOauthFromSearch(window.location.search) : unregisteredOauthFromSearch("");

  // Mode: Sign In vs Create Clinic Account
  const [mode, setMode] = useState<"signin" | "register">(
    bootOauth.provider ? "register" : loginMode || "signin"
  );
  ...
  const [verifiedSsoNotice, setVerifiedSsoNotice] = useState<string | null>(() =>
    bootOauth.provider ? verifiedOauthBanner(bootOauth.provider, bootOauth.email) : null
  );
  const [oauthToken, setOauthToken] = useState(bootOauth.oauthToken);
  const [oauthProvider, setOauthProvider] = useState<"google" | "facebook" | null>(bootOauth.provider);
  const oauthOnboarding = Boolean(oauthToken) || oauthProvider === "google" || oauthProvider === "facebook";
  const [showOauthFallbackPassword, setShowOauthFallbackPassword] = useState(false);
  ...
  const [adminName, setAdminName] = useState(bootOauth.name);
  const [adminEmail, setAdminEmail] = useState(bootOauth.email);
  ...
  useEffect(() => {
    fetch("/api/auth/oauth-config")
      ...
      .catch(() =>
        setOauthConfig({ facebookConfigured: false, googleConfigured: false, sandboxClientOAuthAllowed: false })
      );
  }, []);
  ...
  useEffect(() => {
    ...
    if (oauthError) {
      setError(publicOauthErrorMessage(oauth, oauthError));
    }
    if (params.get("unregistered") === "1") {
      openRegisterForm();
      const ssoEmail = params.get("email") || "";
      const ssoName = params.get("name") || "";
      const ssoToken = params.get("oauthToken") || "";
      if (ssoEmail) setAdminEmail(ssoEmail);
      if (ssoName) setAdminName(ssoName);
      if (ssoToken) setOauthToken(ssoToken);
      setOauthProvider(oauth);
      setVerifiedSsoNotice(verifiedOauthBanner(oauth, ssoEmail));
    }
  }, []);

  useEffect(() => {
    if (!oauthOnboarding || showOtpView || showForgot) return;
    clinicNameRef.current?.focus();
  }, [oauthOnboarding, showOtpView, showForgot]);
  ...
  // Handle Google & Facebook SSO — always the server OAuth 2.0 start routes.
  const handleOAuthSignIn = (provider: "google" | "facebook") => {
    setError("");
    setSuccessMsg("");

    if (provider === "google" && oauthConfig && oauthConfig?.googleConfigured === false) {
      setError("Google Sign-in is not configured. Set GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET.");
      return;
    }

    if (provider === "facebook" && oauthConfig && oauthConfig?.facebookConfigured === false) {
      setError(
        "Facebook Login is not configured. Set FACEBOOK_APP_ID and FACEBOOK_APP_SECRET (or FACEBOOK_CLIENT_ID / FACEBOOK_CLIENT_SECRET)."
      );
      return;
    }
  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await registerClinic({
        ...
        password: oauthOnboarding ? adminPassword.trim() || undefined : adminPassword,
            ...
            {mode === "signin" ? (
              <div className="space-y-4">
                ...
                <div className="pt-3 border-t border-slate-800/80">
                  ...
                  {showGoogleOAuth && showFacebookOAuth && (
                    <div className="mb-3">
                      <SocialOAuthButtons
                        busy={busy}
                        oauthBusy={oauthBusy}
                        onGoogle={() => handleOAuthSignIn("google")}
                        onFacebook={() => handleOAuthSignIn("facebook")}
                        googleTestId="oauth-google-signin"
                        facebookTestId="oauth-facebook-signin"
                      />
                    </div>
                  )}
                </div>
              ...
              >
                {oauthOnboarding && (
                  <div
                    data-testid="register-oauth-verified-banner"
                    className="p-3 rounded-xl bg-emerald-950/40 border border-emerald-600/40 text-emerald-100 text-xs flex items-start gap-2 shadow-sm"
                  >
                    <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0 mt-0.5" />
                    <span className="break-words font-medium">
                      {verifiedSsoNotice ||
                        verifiedOauthBanner(oauthProvider === "google" ? "google" : "facebook", adminEmail)}
                    </span>
                  </div>
                )}
                {verifiedSsoNotice && !oauthOnboarding && (
                  ...
                  <div className="p-3 rounded-xl bg-slate-950 border border-slate-800 text-xs">
                    ...
                    {showGoogleOAuth && showFacebookOAuth && (
                      <SocialOAuthButtons
                        ...
                        onGoogle={() => handleOAuthSignIn("google")}
                        onFacebook={() => handleOAuthSignIn("facebook")}
                        googleTestId="oauth-google-register"
                        facebookTestId="oauth-facebook-register"
                      />
                    )}
                  </div>
                )}
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      type="text"
                      autoComplete={oauthOnboarding ? "off" : "name"}
                      required
                      readOnly={oauthOnboarding}
                      aria-readonly={oauthOnboarding}
                      tabIndex={oauthOnboarding ? -1 : undefined}
                      data-oauth-locked={oauthOnboarding ? "true" : "false"}
                      value={adminName}
                      onChange={(e) => {
                        if (!oauthOnboarding) setAdminName(e.target.value);
                      }}
                      placeholder="Dr. Vikram Malhotra"
                      className={`w-full min-h-11 pl-9 pr-3.5 py-2.5 rounded-xl border text-sm placeholder-slate-500 focus:outline-none ${
                        oauthOnboarding
                          ? "bg-slate-900/80 border-emerald-700/60 text-emerald-100 cursor-not-allowed"
                ...
                {/* Email; WhatsApp sits beside email only on email signup */}
                <div className={oauthOnboarding ? "" : "grid grid-cols-1 lg:grid-cols-2 gap-3"}>
                  <div className="min-w-0">
                    <label className="block text-xs font-semibold text-slate-300 mb-1">
                      Work Email *{oauthOnboarding ? " · verified" : ""}
                    </label>
                    <div className="relative">
                      <Mail className="w-4 h-4 text-slate-400 absolute left-3 top-3" />
                      <input
                        ...
                        type="email"
                        autoComplete={oauthOnboarding ? "off" : "email"}
                        required
                        readOnly={oauthOnboarding}
                        aria-readonly={oauthOnboarding}
                        tabIndex={oauthOnboarding ? -1 : undefined}
                        ...
                        value={adminEmail}
                        onChange={(e) => {
                          if (!oauthOnboarding) setAdminEmail(e.target.value);
                        }}
                        placeholder={REGISTER_EMAIL_PLACEHOLDER}
                        className={`w-full min-h-11 min-w-0 pl-9 pr-3 py-2.5 rounded-xl border text-xs placeholder-slate-500 focus:outline-none ${
                          oauthOnboarding
                            ? "bg-slate-900/80 border-emerald-700/60 text-emerald-100 cursor-not-allowed"
                            : "bg-slate-950 border-slate-700 text-white focus:border-purple-500"
                        }`}
                ...
                {oauthOnboarding && (
                  <>
                    <input type="hidden" name="oauthToken" data-testid="register-oauth-token" value={oauthToken} />
                    <input type="hidden" name="oauthProvider" data-testid="register-oauth-provider" value={oauthProvider || ""} />
                  </>
                )}

                {oauthOnboarding && !showOauthFallbackPassword ? (
                  <button
                    type="button"
                    data-testid="register-oauth-password-toggle"
                    onClick={() => setShowOauthFallbackPassword(true)}
                    className="w-full text-left text-[11px] text-slate-400 hover:text-slate-200 underline-offset-2 hover:underline min-h-11 px-1"
                  >
                    {REGISTER_OAUTH_PASSWORD_LABEL}
                  </button>
                ) : (
                  <div>
                    <label className="block text-xs font-semibold text-slate-300 mb-1" htmlFor="register-password">
                      {oauthOnboarding ? REGISTER_OAUTH_PASSWORD_LABEL : "Secure Master Password *"}
                    </label>
                    <div className="relative">
                      <Lock className="w-4 h-4 text-slate-400 absolute left-3 top-3" />
                      <input
                        ...
                        autoComplete="new-password"
                        required={!oauthOnboarding}
                        minLength={oauthOnboarding ? undefined : 6}
                        value={adminPassword}
                        onChange={(e) => setAdminPassword(e.target.value)}
                        placeholder={oauthOnboarding ? REGISTER_OAUTH_PASSWORD_PLACEHOLDER : REGISTER_PASSWORD_PLACEHOLDER}
                    ...
                    {oauthOnboarding && (
                      <p className="mt-1.5 text-[11px] text-slate-400" data-testid="register-oauth-password-hint">
                        Leave blank to sign in with {oauthProvider === "google" ? "Google" : "Facebook"} only.
                      </p>
                    )}
                  </div>
                )}
