  ...
  useEffect(() => {
    ...
    if (params.get("unregistered") === "1") {
      ...
      setVerifiedSsoNotice(
        `${label} verified your name and email. Those fields are locked. Add clinic details below — a password is optional (fallback for email sign-in).`
      );
    }
  }, []);
          ...
          <div>
            ...
            {mode === "signin" ? (
              ...
              >
                ...
                {/* Password — required for email signup; optional fallback after Facebook/Google */}
                {oauthOnboarding && (
                  <>
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-300 mb-1" htmlFor="register-password">
                    {oauthOnboarding ? "Fallback password for email sign-in" : "Secure Master Password *"}
                  </label>
                  <div className="relative">
                    <Lock className="w-4 h-4 text-slate-400 absolute left-3 top-3" />
                    <input
                  ...
                  {oauthOnboarding && (
                    <p className="mt-1.5 text-[11px] text-slate-400" data-testid="register-oauth-password-hint">
                      Optional. Leave blank to sign in with {oauthProvider === "google" ? "Google" : "Facebook"} only. Set a password if you also want email login.
                    </p>
                  )}
                </div>
