...
export const LoginPage: React.FC = () => {
  ...
  const [email, setEmail] = useState(params.get("demo") === "1" ? "doctor@lumera.me" : "admin@lumera.me");
  ...
  if (user && !busy) {
    return <Navigate to={next || homePathForRole(user.role)} replace />;
  }
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate(next || homePathForRole(signedIn.role), { replace: true });
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-[#14061f] text-white flex items-center justify-center px-4">
