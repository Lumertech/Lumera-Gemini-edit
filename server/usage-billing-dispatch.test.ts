...
describe("Usage wallet billing dispatch paths", () => {
  ...
  it("calendar reminder, book confirmation, receipt, and patient replies meter patient phones with tenantId", async () => {
    ...
    const calendarSrc =
      fs.readFileSync(path.join(__dirname, "whatsapp-calendar.ts"), "utf8") +
      fs.readFileSync(path.join(__dirname, "whatsapp-calendar-reminders.ts"), "utf8");
    ...
    const clinicalSrc = fs.readFileSync(path.join(__dirname, "gemini-clinical.ts"), "utf8");
    ...
    assert.match(clinicalSrc, /blockedAiScribeResponse/);
    assert.match(clinicalSrc, /meterSuccessfulGeminiScribe/);
    assert.match(clinicalSrc, /ai\.models\.generateContent/);
    assert.match(serverSrc, /createUsageBillingRouter/);
    assert.match(clinicalSrc, /clinical-synthesis-engine/);
  });
