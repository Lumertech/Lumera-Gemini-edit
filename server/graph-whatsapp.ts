 ...
 * Authorization: clinic/doctor WABA token first, else MasterAdmin META_GRAPH_TOKEN.
 */
...
export const WHATSAPP_AUTH_MISSING_MESSAGE = "No valid Meta WhatsApp access token available.";

/**
 * Outbound Meta Graph Authorization header.
 * Clinic/doctor WABA token wins (multi-tenant). Otherwise Lumera MasterAdmin
 * META_GRAPH_TOKEN (aliases META_ACCESS_TOKEN / WHATSAPP_ACCESS_TOKEN).
 * Placeholder / demo tokens with `...` do not count.
 */
export function getWhatsAppAuthHeader(
  tenantWabaToken?: string,
  env: NodeJS.ProcessEnv = process.env
): string {
  if (isUsableGraphToken(tenantWabaToken)) {
    return `Bearer ${String(tenantWabaToken).trim()}`;
  }
  const master = platformMetaGraphToken(env);
  if (isUsableGraphToken(master)) {
    return `Bearer ${master}`;
  }
  throw new Error(WHATSAPP_AUTH_MISSING_MESSAGE);
}
...
export async function postGraphWhatsAppMessage(opts: {
  ...
  try {
    const res = await fetchImpl(url, {
      method: "POST",
      headers: {
        Authorization: getWhatsAppAuthHeader(opts.credentials.token),
