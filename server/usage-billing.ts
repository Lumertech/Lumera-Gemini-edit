...
/**
 * PRODUCT TRADEOFF (flagged): client `durationMinutes` is accepted only when it
 * is within ±20% or ±1 minute of the transcript estimate, whichever is larger.
 * Outside that bound we bill the transcript estimate and warn — understated
 * client duration must not reduce billed quantity.
 */
export const SCRIBE_DURATION_TOLERANCE_RATIO = 0.2;
export const SCRIBE_DURATION_TOLERANCE_MINUTES = 1;

export function estimateScribeMinutesFromTranscript(transcript?: string): number {
...
export function scribeQuantityMinutes(body: { durationMinutes?: unknown; transcript?: string }): number {
  const estimated = estimateScribeMinutesFromTranscript(body.transcript);
  const explicit = Number(body.durationMinutes);
  if (!Number.isFinite(explicit) || explicit <= 0) return estimated;
  const slack = Math.max(SCRIBE_DURATION_TOLERANCE_MINUTES, estimated * SCRIBE_DURATION_TOLERANCE_RATIO);
  if (Math.abs(explicit - estimated) <= slack) return explicit;
  console.warn(
    `[Lumera] scribeQuantityMinutes: client durationMinutes=${explicit} is outside ±${SCRIBE_DURATION_TOLERANCE_RATIO * 100}% or ±${SCRIBE_DURATION_TOLERANCE_MINUTES}min of transcript estimate ${estimated}; billing the estimate`
  );
  return estimated;
}
